...
namespace Feliciabot.net._6._0.modules
{
    ...
    public sealed class MusicModule : InteractionModuleBase<SocketInteractionContext>
    {
                ...
                await FollowupAsync(
                        result.Status switch
                        {
                            ...
                            PlayerRetrieveStatus.BotNotConnected
                                => "The bot is currently not connected.",
                            _ => "Unknown error."
                        }
                    )
                    .ConfigureAwait(false);
        ...
        private static ValueTask<CustomPlayer> CreatePlayer(
        ...
        {
            await _player.DisconnectAsync().ConfigureAwait(false);
        ...
        public async Task Play(string query)
        {
            ...
            var position = await _player.PlayAsync(track).ConfigureAwait(false);

            if (position is 0)
            {
                await FollowupAsync(
                        $"🔈 Playing:",
                        embed: await _embedBuilderService.GetTrackInfoFromPlayerAsEmbed(_player)
                    )
                    .ConfigureAwait(false);
            }
            else
            ...
            if (loadedTracks.IsPlaylist)
            {
                ...
                var queueableTracks = remainingTracks
                    ...
                    .ToList()
                    .AsReadOnly();
                await _player.Queue.AddRangeAsync(queueableTracks);
        ...
        {
            if (_player.CurrentTrack == null || _player.IsPaused)
            {
        ...
        {
            if (_player.Queue is null || _player.Queue.Count == 0)
            {
                await RespondAsync("Queue is empty!").ConfigureAwait(false);
                return;
            }

            var paginatedQueue = GetPaginatedQueue(_player.Queue);
        ...
        public async Task Clear()
        {
            ...
            int cleared = await _player.Queue.ClearAsync();
        ...
        public async Task Remove(int trackNumber)
        {
            ...
            if (trackNumber < 1 || trackNumber > _player.Queue.Count)
            {
            ...
            foreach (ITrackQueueItem track in _player.Queue)
            {
            ...
            {
                var removed = await _player.Queue.RemoveAsync(trackToRemove);
                if (removed)
                {
                    await RespondAsync(
        ...
        public async Task Skip()
        {
            ...
            await _player.SkipAsync().ConfigureAwait(false);

            var track = _player.CurrentItem;
        ...
        {
            if (_player.State is PlayerState.Paused)
            {
        ...
        {
            if (_player.State is not PlayerState.Paused)
            {
